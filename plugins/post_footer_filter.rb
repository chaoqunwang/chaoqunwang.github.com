...
#            </p>]
	post.content.gsub(/<!--\s*more\s*-->/i, "<!--more--><a href='#{url}' style='color:black;text-decoration:none;'>#{author}</a>  ") + %Q[<p class='post-footer' style='background:none repeat scroll 0pt 0pt #eee;padding: 5px;margin-top:10px;color:gray;font-size:90%;'>
			&nbsp;#{pre or 'original link:'}
			<a href='#{post.full_url}'>#{post.full_url}</a><br/>
